...
//    private SoftReference<RxActivity> activity;
...
//        this.activity = new SoftReference<>(activity);
    ...
    public Retrofit getReTrofit(int connectTime, String baseUrl) {
        ...
        final Retrofit retrofit = new Retrofit.Builder()
                .client(RxRetrofitApp.okHttpClient)
    ...
    public void httpDeal(Observable observable, BaseApi basePar) {
        ...
        observable = observable.retryWhen(new RetryWhenNetworkException(basePar.getRetryCount(),
                ...
                //.compose(activity.get().bindUntilEvent(ActivityEvent.DESTROY))
                /*返回数据统一判断*/
                .map(new ResulteFunc())
                /*http请求线程*/
        ...
        if (onNextListener != null && null != onNextListener.get()) {
            subscriber = new NetSubscriber(basePar, onNextListener);
     ...
     * @param basePar
     */
    public void httpDealThread(Observable observable, BaseApi basePar) {
          /*失败后的retry配置*/
        observable = observable.retryWhen(new RetryWhenNetworkException(basePar.getRetryCount(),
                basePar.getRetryDelay(), basePar.getRetryIncreaseDelay()))
                /*异常处理*/
